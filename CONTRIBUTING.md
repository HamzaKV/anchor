...
All four must pass. `npm run lint` runs Biome (`biome check .`) over the repo; `npm run format` (`biome format --write .`) applies its formatting fixes in place. `npm test` runs the full vitest suite (unit, integration, regression, e2e); e2e spawns the built CLI, so `npm run build` must run first — `npm test` alone assumes a fresh `dist/`.
